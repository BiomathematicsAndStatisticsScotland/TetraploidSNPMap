...
c	parameter(maxind=200,mxloci=500)
	...
	integer, allocatable,save :: SNPp1(:),SNPp2(:),gtp(:,:)
      integer, allocatable :: mt(:),markpair(:,:),mp(:,:),o(:)
...
c	character locname(mxloci)*20,wkname(mxloci)*20
	...
	real(8) chisqr(100),chist(100),lod
...
140   write (*,*) "Write full output (y/n)?"
	...
	read(3,*) n,nloci
      ...
      allocate (SNPp1(nloci),SNPp2(nloci),gtp(nloci,8),mt(nloci),
...
c	  end do
	  it=0
		if (chiprob.ge.0.001d0) then   !usually 0.001 but changed for sim study
	      it=it+1
		  mt(i)=mt(i)+1
		  prob(i)=chiprob
	      do k=1,8
...
C PRIVATE(outfile1, outfile2, tid, tidnum, l1, markpair, l2, i, nnl, mp, n, gtp, np, lname, g1, g2, gtk)
      CHUNK = 100
!$OMP  PARALLEL
!$OMP& SHARED(wkSNP,pt,wkname,mt,stem,nl2,gtp,n,prob,it,numthr,
!$OMP& fulloutputl)  DEFAULT (PRIVATE)
c      numthr = omp_get_num_threads()
	np=0
	pf=0.0d0
      it2 = it
      tid = 0
...
c	    end do
	    if (mt(l1).gt.0.and.mt(l2).gt.0) then
	          np=np+1
	          lname(1)=wkname(l1)
	          lname(2)=wkname(l2)
		      do k=1,4
...
c     & status="old")
c      do i=1,numthr-1
c          write(tidnum, '(I3.3)') i
c          outfile1 = trim(stem) // tidnum // ".SNPout"
c	    outfile2 = trim(stem) // tidnum // ".SNPpwd"
c	    open(11,file=outfile1,status='old')
c          do
c              read(11, '(A500)', iostat=ioresult) iobuf
c              if(ioresult .ne. 0) exit
c              write(8, '(A)')  trim(iobuf)
c          end do
c          close(11)
c          open(11,file=outfile2,status='old')
c          do
c              read(11, '(A500)', iostat=ioresult) iobuf
c              if(ioresult .ne. 0) exit
c              write(9, '(A)') trim(iobuf)
c          end do
c       close(11)
c      end do
