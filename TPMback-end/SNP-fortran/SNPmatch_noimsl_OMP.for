...
c BB 
c                  write(4,'(i4,2x,3a40)') i,fsimpname(j),wkname(i),
c     &            scname(i)
                  cycle long_loop
             endif
          enddo
...
c BB
c                  write(4,'(i4,2x,3a40)') i,msimpname(j),wkname(i),
c     &            scname(i)
                  cycle long_loop
             endif
          enddo 
	...
	itype=5
	call chimatrix(n,fsimpct,i3to1,fsimpname,bname,fsimp,
     &	bothtest,chisq,sigx)
	...
	itype=5
	call chimatrix(n,msimpct,i3to1,msimpname,bname,msimp,
     &	bothtest,chisq,sigx)
...
c	call cnumb(nloci,ficlson,ficrson,kk,ficlus,fnclus)
      ...
      iclus1=2
      iclus2=nloci-1
c      iMill=1   !single linkage method
      D=1.0d0-fdist
      call Milligan_cluster(nloci,simpname,D,iclus1,iclus2,
     & thresh,ip)
...
!$OMP  PARALLEL
!$OMP& SHARED(n,simp,nsimp,simpct,nsimpct,lpat,phase,chisq1,sigx1,kk)
!$OMP& DEFAULT (PRIVATE)
...
c	************************************************
	subroutine chimatrix(n,fsimpct,i3to1,fsimpname1,bname1,fsimp1,
...
!$OMP  PARALLEL
!$OMP& SHARED(chisq1,cumct,i3to1,fsimpct,lcount,lpat,n,nloci,
!$OMP& ntype,sigx1,
!$OMP& locname,numthr,wkgel, wkgel1) DEFAULT (PRIVATE)

      numthr = omp_get_num_threads()
      tid = omp_get_thread_num()
      write(tidnum, '(I3.3)') tid
      if(tid .ne. 0) then
          outfile = "ReadingTPM" // tidnum // ".bpm"
          open(8+2*tid,file=outfile,status='unknown')
      end if
...
!$OMP DO SCHEDULE(dynamic)
	pair_loop: do l=1,fsimpct
     ...
     &          ((gelp(i,j)-expect(i,j))**2)/expect(i,j)
	    ...
	    if (sigx1(l,m) .lt. 0.001) then
            write (8+2*tid,*)
	      write (8+2*tid,'(a,i4,1x,a,a,i4,1x,a)') 'Pair of loci ',l,
     &	  locname(l),' and ',m,locname(m)
	      write (8+2*tid,*) 'Size of matrix',il,im
	      write (8+2*tid,*) 'Observed counts'
	      do i=1,il
c	        write (8+2*tid,'((<im>(1x,i4)),6x,i6)') 
c     &	    (gelp(i,j),j=1,im),marg1(i)
                do j=1,im
          	        write (8+2*tid, '(1x,i4)', advance='no') gelp(i,j)
                end do
                write(8+2*tid,'(6x,i6)') marg1(i)
                
c              write(8+2*tid, *) "BLA"
            end do
            do j=1,im
                write (8+2*tid,'(1x,i4)', advance='no') marg2(j)
            end do
            
c	      write (8+2*tid,'(<im>(1x,i4))') (marg2(j),j=1,im)
            write (8+2*tid,*)
	      write (8+2*tid,*) 'Sum of total counts', cumcttot
	      write(8+2*tid,'(1x,a,2x,f8.2,2x,a,f8.5)')  
     &	  'Chi-squared statistic ',chisq1(l,m),'Significance',sigx1(l,m)
	    endif
	  end do
      end do pair_loop
!$OMP END DO
...
!$OMP END PARALLEL
      ...
      do i=1,numthr-1
          ...
          do
              read(10, '(A500)', iostat=ioresult) iobuf
	...
	do m=fsimpct+1,fsimpct+i3to1
	  do l=1,fsimpct
	    if (sigx1(l,m) .lt. 0.001) then
	      write(4,'(a40,2x,a40,2x,f8.2,2x,f10.8,2x,a)') 
...
c change program to create subroutine Milligan_cluster and call to it
      ...
      subroutine Milligan_cluster(N,locname,D,ICLUS1,ICLUS2,
     & distlim,IP)
...
1880     CONTINUE
         ...
         DO 2000 L = 1, N
            ...
            IF ((D(NI,L)-D(NJ,L)).LE.0.0) GO TO 2000
            D(NI,L) = D(NJ,L)
            D(L,NI) = D(L,NJ)
