...
c     modified from regular QTL program to do a permutation test

c     need to change to set seed explicitly to make sure same perm data is sued for all 12 chromosomes.
c     9/1/2015 change to allocatable
c     9/1/2015 change to work from .maploc files
c     9/1/2015 change lengths of trait names to 20

c     12/1/2015 cut back output to be in same format as old TPM
c     write permuted data to new output file
...
c	parameter(maxind=400,mxloci=500,mxtrt=500,mxstep=150)
...
c     19/10/2016 interpolation of QTL probs with spline smoother replaces by linear interpolation
	...
	integer, allocatable,save ::  iper(:)		
	integer,allocatable :: p1(:,:),p2(:,:),matpheno(:,:,:),matmatch(:,:)
      integer,allocatable ::  partype(:,:)
	integer,allocatable ::  sortSNP(:,:),id(:)

 	real(8), allocatable :: permmatch(:,:),dist(:)
c	real(8), allocatable :: allct(:),halfct(:),segrat(:,:)
	real(8), allocatable :: mdiff(:),rec(:)
	real(8), allocatable :: trait(:,:),otrait(:),trtperm(:,:),xpermu(:)
	real(8), allocatable :: postprob(:,:,:)
...
c     integer markSNP(maxind,mxloci),SNPp1(500),SNPp2(500)
      integer g1(2,4),g2(2,4),gelp(36,36),ISEED(1)
...
c	integer true(mxloci,4),itype(maxind,mxloci,4)
c	integer cstore(1000,mxloci,8)	
c	integer incc(maxind),icstore(maxind,1000,mxloci,4)
c	real(8) totprob(1000),itotprob(maxind,1000)
c	character icbivboth(maxind,1000)*7,cbivboth(1000)*15

	character stem*12
c      infile1*19,infile2*16,outfile1*34,infile3*19
      character infile1*69,infile2*69,outfile1*69,infile3*69
      character fn*50
	character outfile2*69
      character outfile3*69,stem2*12
...
c	end do
	...
	read(5,*) ntrt
      allocate (trait(ntrt,n),traitlab(ntrt),otrait(n))

      read(5,*) (traitlab(j),j=1,ntrt)
c      write(7,*) (traitlab(j),j=1,ntrt)
	do i=1,n
	  read(5,*) id(i),(trait(j,i),j=1,ntrt)
c	write(7,'(<ntrt>(f8.3))') (trait(j,i),j=1,ntrt)
	end do
...
c      do i=1,ntrt
c          write(*,*) i,traitlab(i)
c      enddo
      write (*,*) "Which trait for permutation test?"
	read (*,'(i3)') itt
      write (*,*) "How many permutations (maximum 500)?"
	read (*,'(i4)') nperm
      write (*,*) "Input seed between 0 and 2147483646"
	read (*,'(i11)') ISEED(1)

      allocate (iper(n))
      allocate (trtperm(nperm,n))
c      ,traitplab(nperm))
...
c	do k=1,nmap
c		  write(7,'(a20,f6.2,2x,4i1,2x,4i1,<n>(1x,i1))') mapname(k),
c     &	dist(k),(p1(k,m),m=1,4),(p2(k,m),m=1,4),
c     &    (sortSNP(j,k),j=1,n)
c	end do
...
c        write(8,*) 'Individual',i, 
c        write(*,'(a, i, i)') 'Individual',i, omp_get_thread_num()
...
c	    write(8,'(4i4,6x,<nmap>i2,6x,i5)') (form(jj,k),k=1,4),
...
c     added 8/10/2012 calculate average weights over all offspring to check for distorted segregation   
c     not needed for permutation test
c      segrat=sum(postprob,dim=1)/(1.0d0*n)
c      write (8,*) 'Average weight for each genotype'
c      do jj=0,nstep
c      write (8,'(36f7.4)') (segrat(jj,kk),kk=1,36)
c      enddo

c     12/1/2015 deallocate unneeded arrays to clear space
      deallocate (p1,p2,matpheno,matmatch,partype,sortSNP,permmatch)
      
c     set up permutations for trait itt 
      otrait(1:n)=trait(itt,1:n)
c      ISEED = 662957
c      CALL RNSET (ISEED)
      do j=1,n
          iper(j)=j
...
c     nperm=300
      do i=1,nperm
c        call rnper(iper)
        call permut(iper,n,n,ISEED)
c	  call dpermu(n,otrait,iper,1,xpermu)
        do j=1,n
            xpermu(j)=otrait(iper(j))
        enddo
        trtperm(i,1:n)=xpermu(1:n)
c	  write(7,*) 'Permutation ',i
c	  do j=1,n
c	    write(7,'(2f9.4)') xpermu(j),trtperm(i,j)
c	  enddo
      enddo
      
c     b traitplab(1:nperm)='p'
c     b write(8,*) nperm
c     b do j=1,n
c     b     write(8,'(i5,2x,<nperm>(es12.4))') j,(trtperm(i,j),i=1,nperm)
c     b enddo
      

      deallocate (trait)
c	write (6,*) 'Start QTL analysis - call SNPprob'
	call SNPprob(n,nmap,nstep,form,postprob,nperm,trtperm)
...
c      do i=1,6
c        write(8,'(2(a2))') (gamtype(i,j),j=1,2)
c	end do
...
c      do i=1,6
c	  write(8,'(6(4a2,3x))') ((zgeno(i,j,k),k=1,4), j=1,6)
c	end do
...
c      do i=1,6
c	  write(8,'(6(i1,3x))') (zpheno(i,j), j=1,6)
c	end do
...
c	do j=1,nstep+1
c	  write(8,'(i5,2x,f8.2,2x,36f10.6)') j,xvec(j),
c     &	  (prlin(i,j),i=1,36)
c	enddo
...
c     *******************************************************************
      subroutine permut(ix,n,k,iseed)
c     program for a random permutation of k out of n objects
c     uses algorithm by Green 1963
c     code in Green 1977 Behaviour Research Methods & Instrumentation 9(6), p559
     
      implicit double precision (a-h,o-z)

      integer iseed(1),ix(n)

c      write(4,*) iseed(1)
c      call random_seed(size=1)
      call random_seed(put=iseed(1:1))
      
      do j=1,k
          call random_number(rn)
c          write(4,*) 'rn = ',rn
          i=int((float(n-j+1))*rn)+j
          it=ix(j)
          ix(j)=ix(i)
          ix(i)=it
      enddo
